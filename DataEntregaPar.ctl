...
Begin VB.UserControl DataEntrega 
   Appearance      =   0  'Flat
   BackColor       =   &H80000004&
   ClientHeight    =   4455
   ClientLeft      =   0
   ...
   Begin VB.CommandButton BotaoOK 
      ...
      Height          =   525
      Left            =   2400
      Picture         =   "DataEntregaPar.ctx":0000
      Style           =   1  'Graphical
      TabIndex        =   1
      Top             =   3804
      Width           =   1005
   ...
   Begin VB.CommandButton BotaoCancela 
      ...
      Height          =   525
      Left            =   4152
      Picture         =   "DataEntregaPar.ctx":015A
      Style           =   1  'Graphical
      TabIndex        =   0
      Top             =   3816
      Width           =   1005
...
Function Trata_Parametros(ByVal objDataEntregaParam As ClassDataEntregaParam, Optional iEnabled As Integer = 1, Optional iOrigemPV As Integer = 0) As Long
...
On Error GoTo Erro_Trata_Parametros
    ...
    Set gcolDatas = objDataEntregaParam.colDataEntrega
    Set gcolDatasPV = objDataEntregaParam.colDataEntregaPV
    
    Produto.Caption = objDataEntregaParam.sProduto
    
    lErro = Traz_Datas_Tela(objDataEntregaParam.sProduto, gcolDatas)
...
Dim objDataEntregaPV As ClassDataEntrega
    ...
    If Len(DataEntrega.ClipText) > 0 Then
        ...
        For iLinha = 1 To objGridItens.iLinhasExistentes
            If iLinha <> GridItens.Row Then
                If StrParaDate(DataEntrega.Text) = StrParaDate(GridItens.TextMatrix(iLinha, iGrid_DataEntrega_Col)) Then gError 199846
            End If
        Next
 
        If Not gcolDatasPV Is Nothing Then
            For Each objDataEntregaPV In gcolDatasPV
                If objDataEntregaPV.dtDataEntrega = StrParaDate(DataEntrega.Text) Then
                    iAchou = 1
                    Exit For
                End If
            Next
            If iAchou = 0 Then gError 199850
        End If
...
Erro_Saida_Celula_DataEntrega:
    ...
    Select Case gErr
        ...
        Case 199846
            Call Rotina_Erro(vbOKOnly, "ERRO_DATAENTREGA_REPETIDA", gErr, iLinha)
            Call Grid_Trata_Erro_Saida_Celula(objGridInt)
            
        Case 199850
            Call Rotina_Erro(vbOKOnly, "ERRO_DATAENTREGA_NAO_EXISTENTE_PV", gErr, GridItens.Row)
            Call Grid_Trata_Erro_Saida_Celula(objGridInt)
...
On Error GoTo Erro_Saida_Celula_Quantidade
    ...
    If Len(Quantidade.Text) > 0 Then
        ...
        If Len(Trim(GridItens.TextMatrix(GridItens.Row, iGrid_DataEntrega_Col))) = 0 Then gError 199844
        
        If Not gcolDatasPV Is Nothing Then
            For Each objDataEntregaPV In gcolDatasPV
                If objDataEntregaPV.dtDataEntrega = StrParaDate(GridItens.TextMatrix(GridItens.Row, iGrid_DataEntrega_Col)) Then
                    If objDataEntregaPV.dQuantidade + QTDE_ESTOQUE_DELTA2 < StrParaDbl(Quantidade.Text) Then gError 199845
                    Exit For
                End If
            Next
...
Erro_Saida_Celula_Quantidade:
    ...
    Select Case gErr
        ...
        Case 199844
            Call Rotina_Erro(vbOKOnly, "ERRO_DATAENTREGA_NAO_PREENCHIDA_GRID", gErr, GridItens.Row)
            Call Grid_Trata_Erro_Saida_Celula(objGridInt)
        
        Case 199845
            Call Rotina_Erro(vbOKOnly, "ERRO_QUANT_MAIOR_QUANTAFATURARDATA", gErr, objDataEntregaPV.dQuantidade)
            Call Grid_Trata_Erro_Saida_Celula(objGridInt)
