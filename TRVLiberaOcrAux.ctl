...
Begin VB.UserControl TRVLiberaOcrAux 
   ClientHeight    =   6000
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   9510
   ...
   Begin VB.CommandButton BotaoOK 
      ...
      Height          =   525
      Left            =   3210
      Picture         =   "TRVLiberaOcrAux.ctx":0000
      Style           =   1  'Graphical
      TabIndex        =   9
      Top             =   5355
      Width           =   1005
   ...
   Begin VB.CommandButton BotaoCancela 
      ...
      Height          =   525
      Left            =   4980
      Picture         =   "TRVLiberaOcrAux.ctx":015A
      Style           =   1  'Graphical
      TabIndex        =   10
      Top             =   5355
      Width           =   1005
   ...
   End
End
Attribute VB_Name = "TRVLiberaOcrAux"
    ...
    For Each objOcr In gcolOcorrencias
        ...
        If objOcr.iOrigem = INATIVACAO_AUTOMATICA_CODIGO Then
            ...
            If objVou.iCartao = MARCADO Then
                ...
                lErro = CF("Coinfo_Obtem_FilialEmp_Cliente", objOcr.lCliente, iFilialEmpCorporator, iFilialEmpCoinfo)
                If lErro <> SUCESSO Then gError 192787
                
                If iFilialEmpCorporator = TRV_FILIAL_BH Then
                    GridItens.TextMatrix(iLinha, iGrid_ISS_Col) = Format(Abs(objOcr.dValorTotal) * 0.02, "STANDARD")
                Else
                    GridItens.TextMatrix(iLinha, iGrid_ISS_Col) = Format(Abs(objOcr.dValorTotal) * 0.05, "STANDARD")
                End If
                
                'Se o voucher está associado a um título
                If objVou.iTipoDocDestino = TRV_TIPO_DOC_DESTINO_TITREC And objVou.lNumIntDocDestino <> 0 Then
                
                    Set objTitRecTRV = New ClassTitulosRecTRV
                    
                    objTitRecTRV.lNumIntDocTitRec = objVou.lNumIntDocDestino
                    ...
                    If lErro <> ERRO_LEITURA_SEM_DADOS Then
                        If objTitRecTRV.dValorBruto <> 0 Then
                            dPercTarifa = objTitRecTRV.dValorTarifa / objTitRecTRV.dValorBruto
                        End If
                    End If

                End If
                
                GridItens.TextMatrix(iLinha, iGrid_TAR_Col) = Format(Abs(objOcr.dValorTotal) * dPercTarifa, "STANDARD")
...
Dim objOcorrenciaAux As ClassTRVOcorrencias
...
On Error GoTo Erro_Move_Tela_Memoria
    ...
    For iLinha = 1 To objGridItens.iLinhasExistentes
        ...
        Set objOcorrenciaAux = New ClassTRVOcorrencias
        
        objOcorrenciaAux.lCodigo = objOcorrencia.lCodigo
        
        'Lê o a Ocorrência
        lErro = CF("TRVOcorrencias_Le", objOcorrenciaAux)
...
Dim lErro As Long
Dim objOrc As ClassTRVOcorrencias
