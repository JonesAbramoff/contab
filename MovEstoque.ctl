...
Begin VB.UserControl MovEstoque 
   ClientHeight    =   6270
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   9510
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.ComboBox TipoMov 
         Appearance      =   0  'Flat
         Height          =   315
         ItemData        =   "MovEstoque.ctx":00EA
         Left            =   4275
         List            =   "MovEstoque.ctx":00EC
         TabIndex        =   11
         Top             =   2130
         Width           =   3270
      End
...
On Error GoTo Erro_BotaoProdutos_Click
    ...
    If GridMovimentos.Col = iGrid_ProdutoOP_Col Then
                
        ' Formata o ProdutoOp
        lErro = CF("Produto_Formata", GridMovimentos.TextMatrix(GridMovimentos.Row, iGrid_ProdutoOP_Col), sProdutoFormatado, iProdutoPreenchido)
        If lErro <> SUCESSO Then gError 52254
        ...
        If Len(Trim(GridMovimentos.TextMatrix(GridMovimentos.Row, iGrid_OP_Col))) <> 0 Then
            ...
            colSelecao.Add Trim(GridMovimentos.TextMatrix(GridMovimentos.Row, iGrid_OP_Col))
    ...
    If lCodigoAntigo <> StrParaLong(Trim(Codigo.Text)) Then
        ...
        If Len(Trim(Codigo.ClipText)) > 0 Then
            ...
            If lErro = SUCESSO Then
                ...
                If Verifica_TipoTelaAtual(objMovEstoque.iTipoMov) <> MOVESTOQUEINT Then gError 41000
...
Dim colSelecao As New Collection
    ...
    Call Chama_Tela("MovEstoqueInternoLista", colSelecao, objMovEstoque, objEventoCodigo)
...
On Error GoTo Erro_From_Load
    ...
    'que têm ValidoMovInt=1 e Inativo=ATIVO
    For Each objTipoMovEst In colTipoMovEst

        If objTipoMovEst.iValidoMovInt = 1 And objTipoMovEst.iInativo = Ativo Then
            
            If giTipoVersao <> VERSAO_LIGHT Then
                
                'Se for versão Full e tipo de Movimento Producao Entrada não colocar na combo
                If objTipoMovEst.iCodigo <> MOV_EST_PRODUCAO And objTipoMovEst.iCodigo <> MOV_EST_PRODUCAO_BENEF3 Then
...
On Error GoTo Erro_objEventoProduto_evSelecao
    ...
    If Len(Trim(GridMovimentos.TextMatrix(GridMovimentos.Row, iGrid_Produto_Col))) = 0 Then
        ...
        If iProdutoPreenchido <> PRODUTO_PREENCHIDO Then
            ...
            lErro = Mascara_RetornaProdutoTela(objProduto.sCodigo, sProdutoMascarado) 'Alterado por Wagner REPLICAR_ACERTO
...
On Error GoTo Erro_Trata_Parametros
    ...
    If Not objMovEstoque Is Nothing Then
        ...
        If lErro <> 30128 Then 'Se ele existe
            ...
            If objTipoMovEst.iValidoMovInt = 0 Then gError 30258
...
''                    If lErro <> SUCESSO And lErro <> 30368 And lErro <> 55316 Then gError 78707
''
''                    If lErro = 30368 Then gError 78708
''
''                    If lErro = 55316 Then gError 78709
''
                    objRastroLote.sCodigo = Lote.Text
                    objRastroLote.sProduto = sProdutoFormatado
...
'        Case 78708
'            Call Rotina_Erro(vbYesNo, "ERRO_OPCODIGO_NAO_CADASTRADO", gErr, objOrdemProducao.sCodigo)
'            Call Grid_Trata_Erro_Saida_Celula(objGridInt)
'
'        Case 78709
...
''            'Verifica se existe a OP
''            lErro = CF("OrdemProducao_Le",objOrdemProducao)
''            If lErro <> SUCESSO And lErro <> 30368 And lErro <> 55316 Then gError 78696
''
''            If lErro = 30368 Then gError 78697
''
''            If lErro = 55316 Then gError 78698
''
            lErro = CF("Produto_Formata", GridMovimentos.TextMatrix(GridMovimentos.Row, iGrid_Produto_Col), sProdutoFormatado, iProdutoPreenchido)
...
On Error GoTo Erro_Saida_Celula_TipoMov
    ...
    If Len(Trim(TipoMov.Text)) <> 0 Then
        ...
        '''??? 11/09/01 Marcelo incluir as demais constantes para a verificação.
        If iCodigo = MOV_EST_PRODUCAO Or iCodigo = MOV_EST_DEV_MATERIAL_PRODUCAO Or iCodigo = MOV_EST_DEV_PROCESSO Then
        
            'Formata o Produto para o BD
            lErro = CF("Produto_Formata", GridMovimentos.TextMatrix(GridMovimentos.Row, iGrid_Produto_Col), sProdutoFormatado, iProdutoPreenchido)
            If lErro <> SUCESSO Then gError 55377
        
            objProduto.sCodigo = sProdutoFormatado
            
            'Lê os demais atributos do Produto
            lErro = CF("Produto_Le", objProduto)
            If lErro <> SUCESSO And lErro <> 28030 Then gError 55378
        
            If lErro = 28030 Then gError 55379
    
            If iProdutoPreenchido = PRODUTO_PREENCHIDO Then
        
                'verifica se o produto pode participar da producao
                If objProduto.iPCP = PRODUTO_PCP_NAOPODE Then gError 55375
                    
                'verifica se o produto é produzivel
                If iCodigo = MOV_EST_PRODUCAO And objProduto.iCompras <> PRODUTO_PRODUZIVEL Then gError 55376
...
On Error GoTo Erro_Saida_Celula_OP
    ...
    If Len(Trim(OPCodigo.Text)) > 0 Then
        ...
        lErro = CF("OrdemProducao_Le", objOrdemProducao)
        If lErro <> SUCESSO And lErro <> 30368 And lErro <> 55316 Then gError 30328

        If lErro = 30368 Then gError 30329

        If lErro = 55316 Then gError 55322
        ...
        If iProdutoOPPreenchido = PRODUTO_PREENCHIDO Then
            ...
            If lErro <> SUCESSO And lErro <> 34711 Then gError 19488

            If lErro = 34711 Then gError 19487
...
On Error GoTo Erro_Saida_Celula_ProdutoOP
    ...
    If Len(Trim(ProdutoOP.ClipText)) > 0 Then
        ...
        lErro = CF("Produto_Critica_Estoque", ProdutoOP.Text, objProduto, iProdutoPreenchido)
        ...
        If Len(Trim(GridMovimentos.TextMatrix(GridMovimentos.Row, iGrid_OP_Col))) > 0 Then
            ...
            lErro = CF("ItemOP_Le", objItemOP)
            If lErro <> SUCESSO And lErro <> 34711 Then gError 19491
            
            'Se não encontrou o Item --> Erro
            If lErro = 34711 Then gError 19490
...
On Error GoTo Erro_Gravar_Registro
    ...
    For iIndice = 1 To objGrid.iLinhasExistentes
        ...
        If iTipoMov = MOV_EST_REQ_PRODUCAO Or iTipoMov = MOV_EST_PRODUCAO Or iTipoMov = MOV_EST_DEV_MATERIAL_PRODUCAO Then

            'Verifica se OPCodigo foi preenchido
            If Len(Trim(GridMovimentos.TextMatrix(iIndice, iGrid_OP_Col))) = 0 Then gError 30359

            If iTipoMov = MOV_EST_REQ_PRODUCAO Or iTipoMov = MOV_EST_DEV_MATERIAL_PRODUCAO Then
                ...
                objOrdemProducao.sCodigo = GridMovimentos.TextMatrix(iIndice, iGrid_OP_Col)
                objOrdemProducao.iFilialEmpresa = giFilialEmpresa

                'Verifica se Produto faz parte da Ordem de Produção ==>>
                lErro = CF("ItensOrdemProducao_Le", objOrdemProducao)
                If lErro <> SUCESSO And lErro <> 30401 Then gError 30395

                '==>> Se não fizer Erro!
                If lErro = 30401 Then gError 30396

                iAchou = 0 'Verifica se o Produto foi encontrado na Ordem de Produção
...
Erro_Gravar_Registro:
    ...
    Select Case gErr
        ...
        Case 30359
            Call Rotina_Erro(vbOKOnly, "ERRO_OP_NAO_PREENCHIDO", gErr, iIndice)
        ...
        Case 30396
            Call Rotina_Erro(vbOKOnly, "ERRO_ORDEMPRODUCAO_SEM_ITENS", gErr, objOrdemProducao.sCodigo, objOrdemProducao.iFilialEmpresa)

        Case 30402
            Call Rotina_Erro(vbOKOnly, "ERRO_PRODUTO_FORA_OP", gErr, GridMovimentos.TextMatrix(iIndice, iGrid_ProdutoOP_Col), iIndice, GridMovimentos.TextMatrix(iIndice, iGrid_OP_Col))
...
'Inicializa o Grid
    ...
    objGridInt.colColuna.Add ("Tipo Movimento") 'Alterado Posição
    objGridInt.colColuna.Add ("Quantidade")
    objGridInt.colColuna.Add ("Custo") 'Inserido por Wagner
    ...
    objGridInt.colCampo.Add (TipoMov.Name) 'Alterado Posição
    objGridInt.colCampo.Add (Quantidade.Name)
    objGridInt.colCampo.Add (Custo.Name) 'Inserido por Wagner
...
On Error GoTo Erro_Preenche_GridMovimentos
    ...
    For Each objItemMovEstoque In colItens
        ...
        'Verifica se TipoMov está ligado a Produção
        If objItemMovEstoque.iTipoMov = MOV_EST_REQ_PRODUCAO Or objItemMovEstoque.iTipoMov = MOV_EST_PRODUCAO Or objItemMovEstoque.iTipoMov = MOV_EST_DEV_MATERIAL_PRODUCAO Then
            ...
            GridMovimentos.TextMatrix(iIndice, iGrid_ProdutoOP_Col) = sProdutoOPMascarado
...
On Error GoTo Erro_Move_Itens_Memoria
    ...
    With GridMovimentos
        ...
        'Verifica se o TipoMov está ligado a produção
        If iTipoMov = MOV_EST_REQ_PRODUCAO Or iTipoMov = MOV_EST_PRODUCAO Or iTipoMov = MOV_EST_DEV_MATERIAL_PRODUCAO Then
        ...
        'Verifica se o MovEstoque foi Estornado
        If .TextMatrix(iIndice, iGrid_Estorno_Col) = "1" Then
            ...
            Select Case iTipoMov
                ...
                Case MOV_EST_REQ_PRODUCAO
                    iTipoMov = MOV_EST_ESTORNO_REQ_PRODUCAO

                Case MOV_EST_PRODUCAO
                    iTipoMov = MOV_EST_ESTORNO_PRODUCAO

                Case MOV_EST_CONSUMO
                    iTipoMov = MOV_EST_ESTORNO_CONSUMO

                Case MOV_EST_DEV_CONSUMO
                    iTipoMov = MOV_EST_ESTORNO_DEV_CONSUMO

                Case MOV_EST_DEV_MATERIAL_PRODUCAO
                    iTipoMov = MOV_EST_ESTORNO_DEV_MATERIAL_PRODUCAO

                Case MOV_EST_PROCESSO
                    iTipoMov = MOV_EST_ESTORNO_PROCESSO

                Case MOV_EST_DEV_PROCESSO
                    iTipoMov = MOV_EST_ESTORNO_DEV_PROCESSO

                Case MOV_EST_OUTRAS_DEV
                    iTipoMov = MOV_EST_ESTORNO_OUTRAS_DEV

                Case MOV_EST_OUTRAS_REQ
                    iTipoMov = MOV_EST_ESTORNO_OUTRAS_REQ
        ...
        Set objItemMovEstoque = objMovEstoque.colItens.Add(colItensNumIntDoc(iIndice), iTipoMov, 0, 0, sProdutoFormatado, .TextMatrix(iIndice, iGrid_Descricao_Col), .TextMatrix(iIndice, iGrid_UnidadeMed_Col), StrParaDbl(.TextMatrix(iIndice, iGrid_Quantidade_Col)), objAlmoxarifado.iCodigo, .TextMatrix(iIndice, iGrid_Almoxarifado_Col), 0, sCclFormatada, CLng(.TextMatrix(iIndice, iGrid_Estorno_Col)), sOPCodigo, sProdutoFormatadoOP, sContaFormatadaAplic, sContaFormatadaEst, 0, colRatreamentoMovto, Nothing, DATA_NULA)
...
Erro_Move_RastroEstoque_Memoria:
    ...
    Select Case gErr
        ...
        Case 78733
            Call Rotina_Erro(vbOKOnly, "ERRO_OP_NAO_PREENCHIDO", gErr, iLinha)
...
Erro_RastreamentoLote_Le:
    ...
    Select Case gErr
        ...
        Case Else
            Call Rotina_Erro(vbOKOnly, "ERRO_FORNECIDO_PELO_VB", gErr, 162864)
...
Erro_RastreamentoMovto_Le_DocOrigem:
    ...
    Select Case gErr
        ...
        Case Else
            Call Rotina_Erro(vbOKOnly, "ERRO_FORNECIDO_PELO_VB", gErr, 162865)
...
Erro_RastreamentoLoteSaldo_Le:
    ...
    Select Case gErr
        ...
        Case 78631, 78632
            Call Rotina_Erro(vbOKOnly, "ERRO_LEITURA_RASTREAMENTOLOTESALDO", gErr, objRastroLoteSaldo.sProduto, objRastroLoteSaldo.iAlmoxarifado, objRastroLoteSaldo.sLote)

        Case 78633 'RastreamentoLoteSaldo não cadastrado

        Case Else
            Call Rotina_Erro(vbOKOnly, "ERRO_FORNECIDO_PELO_VB", gErr, 162866)
    ...
    If objGrid.iLinhasExistentes + 1 = GridMovimentos.Row Then
        ...
        If Len(Trim(GridMovimentos.TextMatrix(GridMovimentos.Row, iGrid_Produto_Col))) = 0 Then
            ...
            If Me.ActiveControl Is Produto Then
                    ...
                    If lErro <> SUCESSO Then gError 210837
            ...
            If objProduto.sCodigoBarras <> "Cancel" Then
                If objProduto.lErro = SUCESSO Then
                    ...
                    If lErro <> SUCESSO Then gError 210838
    
                End If
    
                'Lê os demais atributos do Produto
                lErro = CF("Produto_Le", objProduto)
                If lErro <> SUCESSO And lErro <> 28030 Then gError 210839
    
                'Se não encontrou o Produto --> Erro
                If lErro = 28030 Then gError 210840
    
                lErro = Mascara_RetornaProdutoEnxuto(objProduto.sCodigo, sProdutoEnxuto)
                If lErro <> SUCESSO Then gError 210841
...
'                    'Preenche a Linha do Grid
'                    lErro = ProdutoLinha_Preenche(objProduto)
'                    If lErro <> SUCESSO Then gError 210842
'
'                End If
...
'    GridMovimentos.TextMatrix(GridMovimentos.Row, iGrid_Produto_Col) = ""
    ...
    Select Case gErr
        ...
        Case 210837 To 210839, 210842, 210843, 210868
        ...
        Case Else
            Call Rotina_Erro(vbOKOnly, "ERRO_FORNECIDO_PELO_VB", gErr, Error$, 210844)
