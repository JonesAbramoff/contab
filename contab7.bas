...
Dim iProxSeqAglutinado As Integer, iUltSeqAglutinadoAnt As Integer
Dim lDocAux As Long
...
'        lErro = Comando_ExecutarPos(alComando(4), "SELECT Doc, Seq FROM Lancamentos WHERE FilialEmpresa = ? AND Data = ? AND Origem = ? AND Aglutinado =2 ORDER BY Seq DESC", 0, lDocAglutinado, iUltSeqAglutinadoAnt, iFilialEmpresa, tLancamento.dtData, gcolModulo_sOrigemAglutina(tLancamento.sOrigem))
...
'
'            iUltSeqAglutinadoAnt = 0
'
'        End If
'
'        iUltSeqAglutinadoAnt = iUltSeqAglutinadoAnt + 1
'        iProxSeqAglutinado = iUltSeqAglutinadoAnt + 1
'
'    End If
'
'    Do While lErro = AD_SQL_SUCESSO
...
'                iFilialEmpresa, gcolModulo_sOrigemAglutina(sOrigem), iExercicio, iPeriodo, lDocAglutinado, iUltSeqAglutinadoAnt, 0, iPeriodo, dtData, sConta, sCcl, dValorAglutinado, lDocAglutinado, iUltSeqAglutinadoAnt, LANCAMENTO_AGLUTINADO, objLancamento.sHistorico, iGerencial)
'            If lErro <> AD_SQL_SUCESSO Then gError 203274
'
'            dValorAglutinado = 0
'            iUltSeqAglutinadoAnt = iProxSeqAglutinado
'            iProxSeqAglutinado = iProxSeqAglutinado + 1
'
...
'
'        lErro = Comando_ExecutarPos(alComando(3), "UPDATE Lancamentos SET DocAglutinado = ?, SeqAglutinado = ?", alComando(1), lDocAglutinado, iUltSeqAglutinadoAnt)
...
'            lDocAux, iFilialEmpresa, iExercicio, iPeriodo, tLancamento.lDoc, sOrigem)
'        If lErro <> AD_SQL_SUCESSO Then gError 203271
'
...
'        Do While lErro = AD_SQL_SUCESSO
'
'            lErro = Comando_ExecutarPos(alComando(6), "UPDATE Lancamentos SET DocAglutinado = ?, SeqAglutinado = ?", alComando(5), lDocAglutinado, iProxSeqAglutinado)
'            If lErro <> AD_SQL_SUCESSO Then gError 203276
'
'            iProxSeqAglutinado = iProxSeqAglutinado + 1
'
...
'            iFilialEmpresa, gcolModulo_sOrigemAglutina(sOrigem), iExercicio, iPeriodo, lDocAglutinado, iUltSeqAglutinadoAnt, 0, iPeriodo, dtData, sConta, sCcl, dValorAglutinado, lDocAglutinado, iUltSeqAglutinadoAnt, LANCAMENTO_AGLUTINADO, objLancamento.sHistorico, iGerencial, gsUsuario, Date, CDbl(Time), 0)
...
Erro_LctosAglutinados_Sem_Aglutinador:
    ...
    Select Case gErr
        ...
        Case 203274
            Call Rotina_Erro(vbOKOnly, "ERRO_INSERCAO_LANCAMENTO", gErr, iFilialEmpresa, gcolModulo_sOrigemAglutina(sOrigem), iExercicio, iPeriodo, lDocAglutinado, iUltSeqAglutinadoAnt)
