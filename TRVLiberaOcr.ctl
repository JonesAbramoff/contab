...
Begin VB.UserControl TRVLiberaOcr 
   ClientHeight    =   6000
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   9510
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.ComboBox Forma 
         Height          =   315
         ItemData        =   "TRVLiberaOcr.ctx":0000
         Left            =   1755
         List            =   "TRVLiberaOcr.ctx":000A
   ...
   End
End
Attribute VB_Name = "TRVLiberaOcr"
...
Dim gobjTRVLiberaOcrSel As New ClassTRVLiberaOCRSel
...
Private Function Move_Selecao_Memoria(ByVal objTRVLiberaOcrSel As ClassTRVLiberaOCRSel) As Long
...
On Error GoTo Erro_Move_Selecao_Memoria
    ...
    If OptVouPag.Value Then
        objTRVLiberaOcrSel.iPago = TRV_VOU_PAGO
    ElseIf OptVouNaoPag.Value Then
        objTRVLiberaOcrSel.iPago = TRV_VOU_NAO_PAGO
    Else
        objTRVLiberaOcrSel.iPago = TRV_VOU_PAGO_E_NAO_PAGO
    End If

    objTRVLiberaOcrSel.dtDataEmissaoAte = StrParaDate(DataEmissaoAte.Text)
    objTRVLiberaOcrSel.dtDataEmissaoDe = StrParaDate(DataEmissaoDe.Text)
    objTRVLiberaOcrSel.lCodigoAte = StrParaLong(OcorrenciaAte.Text)
    objTRVLiberaOcrSel.lCodigoDe = StrParaLong(OcorrenciaDe.Text)
    
    If objTRVLiberaOcrSel.dtDataEmissaoAte <> DATA_NULA And objTRVLiberaOcrSel.dtDataEmissaoDe <> DATA_NULA Then
        If objTRVLiberaOcrSel.dtDataEmissaoAte < objTRVLiberaOcrSel.dtDataEmissaoDe Then gError 190256
    End If
...
Dim objTRVLiberaOcrSel As New ClassTRVLiberaOCRSel
...
On Error GoTo Erro_Traz_Bloqueios_Tela
    ...
    lErro = Move_Selecao_Memoria(objTRVLiberaOcrSel)
    If lErro <> SUCESSO Then gError 190258
  
    'Preenche a Coleção de Bloqueios
    lErro = CF("TRVOcorrencias_Le_Bloqueios", objTRVLiberaOcrSel)
    If lErro <> SUCESSO Then gError 190259
    
    If Not bTirandoLiberados Then
        If objTRVLiberaOcrSel.colOcorrenciais.Count = 0 Then gError 190260
    End If
    
    Set gobjTRVLiberaOcrSel = objTRVLiberaOcrSel
    
    'Preenche o GridBloqueio
    lErro = Grid_Bloqueio_Preenche(objTRVLiberaOcrSel)
...
Dim lErro As Long
Dim objOcorrencia As New ClassTRVOcorrencias
...
On Error GoTo Erro_BotaoOCR_Click
    ...
    objOcorrencia.lCodigo = gobjTRVLiberaOcrSel.colOcorrenciais.Item(GridBloqueio.Row).lCodigo

    'Chama a tela de Pedidos de Venda
    Call Chama_Tela("TRVOcorrencias", objOcorrencia)
...
Public Sub Form_Unload(Cancel As Integer)
    ...
    Set gobjTRVLiberaOcrSel = Nothing
...
Dim colcolColecoes As New Collection
    ...
    colcolColecoes.Add gobjTRVLiberaOcrSel.colOcorrenciais
...
Dim objOcorrencia As New ClassTRVOcorrencias
    ...
    Call Chama_Tela("TRVOcorrenciaLista", colSelecao, objOcorrencia, objEventoOcrAte)
...
Dim objOcorrencia As New ClassTRVOcorrencias
    ...
    'Chama Tela PedidoVendaLista
    Call Chama_Tela("TRVOcorrenciaLista", colSelecao, objOcorrencia, objEventoOcrDe)
...
On Error GoTo Erro_OcorrenciaDe_Validate
    ...
    If Len(Trim(OcorrenciaDe.Text)) > 0 Then
        ...
        lErro = CF("TRVOcorrencias_Le", objOcorrencia)
...
Erro_OcorrenciaDe_Validate:
    ...
    Select Case gErr
        ...
        Case 190276
            Call Rotina_Erro(vbOKOnly, "ERRO_TRVOCORRENCIAS_NAO_CADASTRADO", gErr, objOcorrencia.lCodigo)
...
Function Trata_Parametros(Optional objOcorrencia As ClassTRVOcorrencias) As Long
...
Private Function Grid_Bloqueio_Preenche(ByVal objLiberaOcrSel As ClassTRVLiberaOCRSel) As Long
...
'    If lErro <> SUCESSO Then gError ERRO_SEM_MENSAGEM
    ...
    If bAbreTela Then
        Call Chama_Tela_Modal("TRVLiberaOcrAux", colOcorrencias)
        If giRetornoTela <> vbOK Then gError ERRO_SEM_MENSAGEM
    End If
    
    'Libera os Bloqueios selecionados
    lErro = CF("TRVOcorrencias_Libera", colOcorrencias)
    If lErro <> SUCESSO Then gError ERRO_SEM_MENSAGEM
...
On Error GoTo Erro_Move_Tela_Memoria
    ...
    For iIndice = 1 To objGridBloqueio.iLinhasExistentes
        ...
        If GridBloqueio.TextMatrix(iIndice, iGrid_Libera_Col) = GRID_CHECKBOX_ATIVO Then
            ...
            Set objOcorrencia = gobjTRVLiberaOcrSel.colOcorrenciais.Item(iIndice)
