...
Begin VB.UserControl RomaneioGrade2 
   ClientHeight    =   8505
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   8340
   ...
   Begin VB.CommandButton BotaoCancela 
      ...
      Height          =   525
      Left            =   4170
      Picture         =   "RomaneioGrade2.ctx":0000
      Style           =   1  'Graphical
      TabIndex        =   53
      Top             =   7890
      Width           =   1005
   ...
   Begin VB.CommandButton BotaoOK 
      ...
      Height          =   525
      Left            =   2760
      Picture         =   "RomaneioGrade2.ctx":0102
      Style           =   1  'Graphical
      TabIndex        =   52
      Top             =   7890
      Width           =   1005
   ...
   End
End
Attribute VB_Name = "RomaneioGrade2"
...
Dim objItemRomaneioGrade As ClassItemRomaneioGrade
Dim objProd As ClassProduto, colItensCat As Collection, colcolItensCat As New Collection
            ...
            For Each objCategProdItemAux In objItemRomaneiro.colCategoria
                If UCase(objCategProdItemAux.sCategoria) = UCase(objGradeCateg.sCategoria) Then
                    ...
                    For Each objCategProdItem In colItensCategProd
                        If UCase(objCategProdItem.sItem) = UCase(objCategProdItemAux.sItem) Then
                            bAchou = True
                        End If
                    Next
                    If Not bAchou Then
                        Set objCategProdItem = New ClassCategoriaProdutoItem
                        objCategProdItem.sCategoria = objCategProdItemAux.sCategoria
                        objCategProdItem.sItem = objCategProdItemAux.sItem
                        colItensCategProd.Add objCategProdItem
                    End If
                End If
            Next
        Next
...
'
'        Set colItensCat = New Collection
'        For Each objCategProdItem In colItensCategProd
'
...
'                    For Each objCategProdItemAux In objItemRomaneiro.colCategoria
'                        If objCategProdItemAux.sCategoria = objGradeCateg.sCategoria Then
'                            If objCategProdItemAux.sItem = objCategProdItem.sItem Then
'                                bAchou = True
'                                Exit For
'                            End If
'                        End If
...
'                colItensCat.Add objCategProdItem
'            End If
'
'        Next
'
'        colcolItensCat.Add colItensCat
        
        colcolItensCat.Add colItensCategProd
        
        objGridGrade.colColuna.Add (objGradeCateg.sCategoria)
    
    Next
    
    objGridGrade.colColuna.Add ("Quantidade")
    
    iIndice = 0
    For Each objGradeCateg In objGrade.colCategoria
        iIndice = iIndice + 1
        
        Load Categoria(iIndice)
       
        'Traz o controle recem desenhado para a frente
        Categoria(iIndice).ZOrder
        'Torna o controle visível
        Categoria(iIndice).Visible = True
        'Informa ao objGrid que esse controle é dessa coluna
        objGridGrade.colCampo.Add (Categoria(iIndice).Name)
        'Informa ao objGrid qual é o indice desse controle no array criado
        objGridGrade.colIndex.Add iIndice
        
        Set colItensCat = colcolItensCat.Item(iIndice)
        
        Categoria(iIndice).Clear
        For Each objCategProdItem In colItensCat
            Categoria(iIndice).AddItem objCategProdItem.sItem
        Next
        
    Next
    
    Quantidade(0).Visible = True
    objGridGrade.colCampo.Add (Quantidade(0).Name)
    objGridGrade.colIndex.Add 0
    
    iGrid_Qtd_Col = iIndice + 1
    
    'todas as linhas do grid
    objGridGrade.objGrid = GridGrade
    objGridGrade.objGrid.Rows = 100 + 1
    
    objGridGrade.iLinhasVisiveis = 5
...
Dim objCategProdItem As ClassCategoriaProdutoItem, iColuna As Integer
    ...
    For Each objItemRomaneio In gobjRomaneioGrade.colItensRomaneioGrade
        If objItemRomaneio.dQuantidade > 0 Then
            ...
            For Each objCategProdItem In objItemRomaneio.colCategoria
                ...
                iIndice = 0
                For iColuna = 1 To iGrid_Qtd_Col - 1
                    If UCase(GridGrade.TextMatrix(0, iColuna)) = UCase(objCategProdItem.sCategoria) Then
                        iIndice = iColuna
                        Exit For
                    End If
                Next
                
                GridGrade.TextMatrix(iNumLinhas, iIndice) = objCategProdItem.sItem
            Next
            GridGrade.TextMatrix(iNumLinhas, iGrid_Qtd_Col) = Formata_Estoque(objItemRomaneio.dQuantidade)
        End If
    Next
    
    objGridGrade.iLinhasExistentes = iNumLinhas
...
Public Sub Obtem_ItemRomaneio_Linha(ByVal iLinha As Integer, iIndexObj As Integer, objItemRomaneio As ClassItemRomaneioGrade)

Dim objCategProdItem As ClassCategoriaProdutoItem
Dim bAchou As Boolean
Dim objItemRomaneioAux As ClassItemRomaneioGrade
Dim iIndexObjAux As Integer
Dim iIndice As Integer, iColuna As Integer

    iIndexObj = 0
    iIndexObjAux = 0
    For Each objItemRomaneioAux In gobjRomaneioGrade.colItensRomaneioGrade
    
        iIndexObjAux = iIndexObjAux + 1
        iIndice = 0
        bAchou = True
        
        For Each objCategProdItem In objItemRomaneioAux.colCategoria
            ...
            If GridGrade.TextMatrix(iLinha, iIndice) <> objCategProdItem.sItem Then
                bAchou = False
                Exit For
            End If
        ...
        Next
        If bAchou Then
            Set objItemRomaneio = objItemRomaneioAux
            iIndexObj = iIndexObjAux
            Exit For
        End If
    Next
End Sub

Public Sub Traz_Item_Tela(ByVal iLinha As Integer)

Dim lErro As Long
Dim iIndice As Integer
Dim objItemRomaneio As New ClassItemRomaneioGrade
Dim sProdutoMascarado As String, iIndex As Integer, bFlag As Boolean
Dim bJaExisteCats As Boolean, iLinhaAux As Integer, iColunaAux As Integer, bDif As Boolean

On Error GoTo Erro_Traz_Item_Tela
    ...
    Call Obtem_ItemRomaneio_Linha(iLinha, iIndex, objItemRomaneio)
    
    'Busca a mesma configuração de categorias nas demais linhas
    bJaExisteCats = False
    For iLinhaAux = 1 To objGridGrade.iLinhasExistentes
        If iLinhaAux <> iLinha Then
            bDif = False
            For iColunaAux = 1 To iGrid_Qtd_Col - 1
                If GridGrade.TextMatrix(iLinhaAux, iColunaAux) <> GridGrade.TextMatrix(iLinha, iColunaAux) Then
                    bDif = True
                End If
            Next
            If Not bDif Then
...
'               exit for
    ...
    'Se encontrou
    If iIndex > 0 And Not bJaExisteCats Then
        
        'GUarda que ele é o item atual
        gobjRomaneioGrade.iItemAtual = iIndex
        ...
        For iIndice = 1 To iGrid_Qtd_Col - 1
           Categoria(iIndice).Enabled = bFlag
        Next
        Quantidade(0).Enabled = bFlag
        ...
        Select Case gobjRomaneioGrade.iModoFuncionamento
            ...
            Case ROMANEIOGRADE_FUNCIONAMENTO_OP
                ...
                lErro = Rotina_Grid_Enable_OP(objItemRomaneio, Quantidade(0))
                If lErro <> SUCESSO Then gError 126617
                    
            Case ROMANEIOGRADE_FUNCIONAMENTO_PRODSAI
        
                lErro = Rotina_Grid_Enable_ProdSai(objItemRomaneio, Quantidade(0))
                If lErro <> SUCESSO Then gError 126618
                    
            Case ROMANEIOGRADE_FUNCIONAMENTO_PRODENT
        
                lErro = Rotina_Grid_Enable_ProdEnt(objItemRomaneio, Quantidade(0))
...
Erro_Traz_Item_Tela:
    ...
    Select Case gErr
        ...
        Case 208213
            Call Rotina_Erro(vbOKOnly, "ERRO_PRODUTOGRADE_CAT_JA_SELECIONADAS", gErr, iLinhaAux)
...
Private Sub GridGrade_RowColChange()
    ...
    Call Traz_Item_Tela(GridGrade.Row)
...
On Error GoTo Erro_Saida_Celula
    ...
    If lErro = SUCESSO Then
        ...
        If GridGrade.Col = iGrid_Qtd_Col Then
            lErro = Saida_Celula_Quantidade(objGridGrade)
            If lErro <> SUCESSO Then gError 123221
        Else
            lErro = Saida_Celula_Categoria(objGridGrade, Categoria(GridGrade.Col))
            If lErro <> SUCESSO Then gError 123221
        End If
...
Private Function Saida_Celula_Categoria(objGridInt As AdmGrid, objControle As Object) As Long
'Faz a crítica da célula Quantidadeque está deixando de ser a corrente

Dim lErro As Long
Dim iLinhaAux As Integer, iLinha As Integer
Dim bDif As Boolean, iColunaAux As Integer, sCategoriaLinha As String

On Error GoTo Erro_Saida_Celula_Categoria

    Set objGridInt.objControle = objControle
    
    iLinha = GridGrade.Row

    'Se o controle tem alguma coisa escrita
    If Len(Trim(objControle.Text)) > 0 Then

        If objControle.Index = iGrid_Qtd_Col - 1 Then
            ...
            For iLinhaAux = 1 To objGridGrade.iLinhasExistentes
                If iLinhaAux <> iLinha Then
                    ...
                    For iColunaAux = 1 To iGrid_Qtd_Col - 1
                        ...
                        If iColunaAux = objControle.Index Then
                            sCategoriaLinha = objControle.Text
                        Else
                            sCategoriaLinha = GridGrade.TextMatrix(iLinha, iColunaAux)
                        End If
                    
                        If GridGrade.TextMatrix(iLinhaAux, iColunaAux) <> sCategoriaLinha Then
                            bDif = True
                        End If
                    Next
                    If Not bDif Then gError 208213
                End If
            Next
...
On Error GoTo Erro_Saida_Celula_Quantidade
    ...
    Set objGridInt.objControle = Quantidade(0)
    ...
    'Se o controle tem alguma coisa escrita
    If Len(Quantidade(0)) > 0 Then

        'Valida se é é um valor e positivo
        If objTipoDocInfo.iComplementar = DESMARCADO Then
            lErro = Valor_Positivo_Critica(Quantidade(0).Text)
        Else
            lErro = Valor_NaoNegativo_Critica(Quantidade(0).Text)
        End If
        'lErro = Valor_Positivo_Critica(Quantidade(0).Text)
        If lErro <> SUCESSO Then gError 26665

        'Formata o valor para exibição
        Quantidade(0).Text = Formata_Estoque(Quantidade(0).Text)
    ...
    dQuantidade = StrParaDbl(Quantidade(0).Text)
    ...
    dQuantAnterior = StrParaDbl(GridGrade.TextMatrix(GridGrade.Row, iGrid_Qtd_Col))
    
    'Se a quantidade for igua a anterior ==> Sinaliza A igualdade
    If dQuantAnterior = StrParaDbl(Quantidade(0).Text) Then bQuantidadeIgual = True
    
    'EScreve no grid
    GridGrade.TextMatrix(GridGrade.Row, iGrid_Qtd_Col) = Formata_Estoque(dQuantidade)
...
On Error GoTo Erro_VisualizarFigura
    ...
    If Len(Trim(sProduto)) > 0 Then
        ...
        If Len(Trim(sFigura)) = 0 And objProduto.iNivel > 1 Then
...
Private Sub Categoria_Change(Index As Integer)

    iAlterado = REGISTRO_ALTERADO

End Sub

Private Sub Categoria_GotFocus(Index As Integer)
    Call Grid_Campo_Recebe_Foco(objGridGrade)
End Sub

Private Sub Categoria_KeyPress(Index As Integer, KeyAscii As Integer)
    Call Grid_Trata_Tecla_Campo(KeyAscii, objGridGrade)
End Sub

Private Sub Categoria_Validate(Index As Integer, Cancel As Boolean)

Dim lErro As Long

    Set objGridGrade.objControle = Categoria(Index)
    lErro = Grid_Campo_Libera_Foco(objGridGrade)
    If lErro <> SUCESSO Then Cancel = True
